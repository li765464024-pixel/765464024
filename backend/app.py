...
def api_llm_status():
    """检查 LLM 服务状态 (通过探测端点是否可达)"""
    import requests
    try:
        from backend.services.llm_client import FREELM_BASE_URL, FREELM_API_KEY
        base = FREELM_BASE_URL.rstrip('/v1').rstrip('/')
        headers = {"Authorization": f"Bearer {FREELM_API_KEY}"} if FREELM_API_KEY else {}
        # 禁用代理，确保直连 localhost
        r = requests.get(f"{base}/v1/models", headers=headers, timeout=3, proxies={"http": "", "https": ""})
        ...
        if connected:
            ...
            return jsonify({
                ...
                'data': {
                    'connected': True,
                    'message': f'LLM 服务正常，可用模型 {model_count}+',
                    'model': 'FreeLLMAPI Router',
                }
            })
        else:
            return jsonify({
                ...
                'data': {
                    'connected': False,
                    'message': f'LLM 服务离线 (HTTP {r.status_code})',
                    'model': '',
                }
            })
        ...
        return jsonify({
            ...
            'data': {
                'connected': False,
                'message': 'LLM 服务未运行，请启动 FreeLLMAPI (localhost:3001)',
                'model': '',
            }
        })
    except Exception as e:
...
def api_llm_analyze_sentiment():
    ...
    # 获取相关帖子
    posts = query(
        "SELECT title, author, direction, content FROM posts WHERE date=? AND (title LIKE ? OR content LIKE ?) ORDER BY id DESC LIMIT 15",
        (analysis_date, f'%{topic}%', f'%{topic}%')
    )
    
    try:
        from backend.services.llm_client import analyze_topic_sentiment
        result = analyze_topic_sentiment(topic, posts)
        return jsonify({'ok': True, 'data': result, 'post_count': len(posts)})
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e)})


@app.route('/api/llm/daily-strategy', methods=['POST'])
def api_llm_daily_strategy():
    """生成次日操作策略"""
    analysis_date = request.args.get('date', date.today().strftime("%Y-%m-%d"))
    
    try:
        from backend.services.llm_client import generate_daily_strategy
        from backend.services.hot_topic_scorer import compute_all_rankings
        
        # 获取市场数据
        market = query("SELECT * FROM market_data WHERE date=? ORDER BY id DESC LIMIT 1", (analysis_date,))
        m = market[0] if market else {}
        
        # 获取排行榜
        rankings = compute_all_rankings(analysis_date)
        hot_topics = rankings.get('mainline_rankings', [])[:5]
        
        # 板块数据
        sectors = query("SELECT sector, COUNT(*) as cnt FROM zt_stocks WHERE date=? AND sector!='' GROUP BY sector ORDER BY cnt DESC LIMIT 10", (analysis_date,))
        
        strategy = generate_daily_strategy(m, hot_topics, sectors)
        return jsonify({'ok': True, 'data': {'strategy': strategy, 'date': analysis_date}})
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e)})
